...
- **Generation:** Responses API with a request-specific Zod schema restricting citation IDs and line bounds to the supplied context, `store: false`, and a bounded output. Model source IDs are resolved to server-owned paths. Each section needs an in-range citation; invalid/missing citations reject the generated answer. Repeated inline references are accepted only when they agree with that section's validated citations. Refusals and incomplete outputs receive explicit error handling. This follows the [official Structured Outputs pattern](https://developers.openai.com/api/docs/guides/structured-outputs).
...
- Strengthen untrusted cloning with process/container isolation and enforce download/disk/time limits before and during the clone. The current clone already excludes inherited system/global Git configuration and injected Git environment settings.
